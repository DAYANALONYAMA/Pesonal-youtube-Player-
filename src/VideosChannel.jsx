...
export function VideosChannel() {
  ...
  useEffect(() => {
    fetch(
      `https://youtube.googleapis.com/youtube/v3/search?type=video&key=${API}&channelId=${id}&part=snippet&maxResults=20`,
      {
        method: "GET",
        ...
        <div className="videos">
          {video &&
            video.map((data, index) => {
              return (
                <Link className="Link" to={`/VideosPlaying/${data.id.videoId}`}>
                  <div key={index}>
                    <img
